...
namespace Lab4_ShoppingCart
{
	public partial class Form1 : Form
	{
		...
		private void btnAddtoCart_Click(object sender, EventArgs e)
		{
			...
			ProductItem productItem = new ProductItem(selectedProduct, (int)nudQty.Value);

			lbCart.Items.Add(productItem);
			if (btnClearCart.Enabled == false) btnClearCart.Enabled = true;

			tbTotal.Text = (int.Parse(tbTotal.Text) + productItem.TotalPrice).ToString();
		}

		private void btnDeleteFromCart_Click(object sender, EventArgs e)
		{
			ProductItem selectedProductItem = lbCart.SelectedItem as ProductItem;
			lbCart.Items.RemoveAt(lbCart.SelectedIndex);
			btnClearCart.Enabled = lbCart.Items.Count != 0;

			tbTotal.Text = (int.Parse(tbTotal.Text) - selectedProductItem.TotalPrice).ToString();
		}
